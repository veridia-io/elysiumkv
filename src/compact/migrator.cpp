...
std::optional<Migration> pick_migration(const Version& version, const ResolvedTiers& tiers,
    ...
    for (const auto& [tier_index, contents] : by_tier) {
        ...
        for (const FileMetadata& file : contents.files) {
            if (placement(tiers, file.min_write_time_ms, file.file_bytes, now_ms) <= tier_index) {
                continue;
            }
        ...
        if (candidate != nullptr) {
            return Migration{*candidate, tier_index,
                             placement(tiers, candidate->min_write_time_ms,
                                       candidate->file_bytes, now_ms),
    ...
    for (const auto& [tier_index, contents] : by_tier) {
        for (const FileMetadata& file : contents.files) {
            const int target = placement(tiers, file.min_write_time_ms, file.file_bytes, now_ms);
