...
///
/// **What the merge is given instead of a child per file.** Building an iterator used to open every
/// file that could overlap the range — an `SstReader` each, with its index block, and on a remote
/// tier a round trip each — before the caller had read one entry. Measured at 186 µs over 697 files
/// for a store that answers the first key from one of them.
///
