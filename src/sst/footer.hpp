...
struct Footer {
    ...
    /// Every block carries a CRC, so damage inside one is caught where it happens. Without this,
    /// damage in the footer produces plausible handles and the read fails later at a block that was
    /// never the problem. Covers
